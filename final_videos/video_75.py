...
NEW_SQUARE_INTERVAL = 0.05  # Initial time interval in seconds to add new square
...
SQUARE_CREATION_ACCELERATION = 1.005  # Factor to decrease interval for square creation after each bounce
...
balls = [
    Ball([WIDTH // 2 + 33, HEIGHT // 2], [random.choice([-MAX_SPEED, MAX_SPEED]), random.choice([-MAX_SPEED, MAX_SPEED])]),
    Ball([WIDTH // 2 - 33, HEIGHT // 2], [random.choice([-MAX_SPEED, MAX_SPEED]), random.choice([-MAX_SPEED, MAX_SPEED])]),
...
# Spark class to handle sparks
class Spark:
    def __init__(self, pos, angle, speed, life):
        self.pos = pos
        self.angle = angle
        self.speed = speed
        self.life = life
...
def create_sparks(pos, num_sparks=10):
    for _ in range(num_sparks):
        angle = random.uniform(0, 2 * math.pi)
        speed = random.uniform(2, 5)
        life = random.randint(10, 20)
        sparks.append(Spark(list(pos), angle, speed, life))
    ...
    if not game_over:
        for ball in balls:
            ...
            if ball.pos[0] <= BALL_RADIUS or ball.pos[0] >= WIDTH - BALL_RADIUS:
                ...
                create_sparks(ball.pos)  # Create sparks on bounce
                ...
                for square in squares[:]:
                    ...
                    for edge in edges:
                        ...
                        if dist <= BALL_RADIUS:
                            ...
                            play_piano_notes()  # Play piano notes on bounce
                            create_sparks(ball.pos)  # Create sparks on collision
        ...
        if squares:
            ...
            if current_time - last_square_add_time >= NEW_SQUARE_INTERVAL:
                ...
                rotation_speed *= -1  # Reverse rotation direction
