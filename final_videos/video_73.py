...
CIRCLE_SHRINK_RATE = 0.985  # Reduced shrink rate
...
# Constants
CIRCLE_SHRINK_RATE = 0.985  # Slightly increased shrink rate to slow down shrinking
...
while running:
    ...
    if not game_over:
        if elapsed_time >= GAME_DURATION and not show_end_message:
        ...
        if ball_pos[0] - BALL_RADIUS <= 0 or ball_pos[0] + BALL_RADIUS >= WIDTH:
            ...
            if no_circle_time is None:
                ...
                circle.radius *= CIRCLE_SHRINK_RATE  # Shrink circle on bounce
        ...
        if ball_pos[1] - BALL_RADIUS <= 0 or ball_pos[1] + BALL_RADIUS >= HEIGHT:
            ...
            if no_circle_time is None:
                bounce_count += 1
                trail_length += 1  # Increase trail length
                BALL_RADIUS *= 1.0033  # Increase ball size with each bounce
                print(BALL_RADIUS)
                circle.radius *= CIRCLE_SHRINK_RATE  # Shrink circle on bounce

        # Check collision with the circle
        dist = math.hypot(ball_pos[0] - WIDTH // 2, ball_pos[1] - HEIGHT // 2)
        if dist + BALL_RADIUS >= circle.radius and (current_time - last_collision_time) > COLLISION_COOLDOWN:
            ...
            if no_circle_time is None:
                ...
                bounce_count += 1
                trail_length += 1  # Increase trail length
                circle.radius *= CIRCLE_SHRINK_RATE  # Shrink circle on collision

    # Update circle color every frame without changing size
    circle.color = get_hue_color(hue_value)
    ...
    if not game_over or show_end_message:
        ...
        title_text = font.render("Increasing in size Ball in Shrinking Circle", True, WHITE)
        screen.blit(title_text, (WIDTH // 2 - title_text.get_width() // 2, 125))

        # Add "UNTIL IT BREAKS" text below the title
        subtitle_text = font.render("UNTIL IT BREAKS", True, WHITE)
        screen.blit(subtitle_text, (WIDTH // 2 - subtitle_text.get_width() // 2, 175))
        ...
        # Draw trail
        for pos, radius in trail_positions:
            pygame.draw.circle(screen, WHITE, pos, radius, 1)
            pygame.draw.circle(screen, BLACK, pos, radius - 1)

        # Draw circle above the trail
        if not game_over and circle.radius > 0:
            circle.draw(screen)

        # Draw ball with circle's hue color
        ball_color = circle.color
        pygame.draw.circle(screen, ball_color, ball_pos, BALL_RADIUS)

        # Update hue value for the next frame
        hue_value += 2
