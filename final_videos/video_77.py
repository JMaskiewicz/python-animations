...
FPS = 60
TRAIL_LENGTH = 7
TRIANGLE_GROWTH_RATE = 3  # Hexagons grow
NEW_TRIANGLE_INTERVAL = 0.3
MIN_TRIANGLE_SIDE = 10  # Small hexagons start small
...
# Hexagon class (formerly Triangle)
class Hexagon:
    ...
    def draw(self, screen):
        if self.side_length < 700:  # Adjust the condition if needed
            half_length = self.side_length / 2
            center = (WIDTH // 2, HEIGHT // 2)

            vertices = [
                (center[0] + half_length * math.cos(self.angle + math.pi / 3 * i),
                 center[1] + half_length * math.sin(self.angle + math.pi / 3 * i))
                for i in range(6)
            ]
...
# Initialize hexagons list and track time for hexagon creation
...
while running:
    ...
    if not game_over:
        ...
        # Spawn a new hexagon at the center based on the NEW_TRIANGLE_INTERVAL
        if current_time - last_hexagon_add_time >= NEW_TRIANGLE_INTERVAL:
            hue = (hue + 0.075) % 1
            rgb_color = colorsys.hsv_to_rgb(hue, 1.0, 1.0)
            hexagon_color = (int(rgb_color[0] * 255), int(rgb_color[1] * 255), int(rgb_color[2] * 255))
            hexagons.append(Hexagon(10, hexagon_color, rotation_hexagon))  # Start small
            rotation_hexagon += 0.0005
        ...
        for hexagon in hexagons:
            hexagon.update()
        hexagons = [hexagon for hexagon in hexagons if hexagon.side_length < 700]
        ...
        # Check for collisions between squares and hexagons
        for square in squares[:]:
            square_edges = square.get_edges()
            for hexagon in hexagons[:]:
                hexagon_edges = hexagon.get_edges()
                collision_detected = False
                for square_edge in square_edges:
                    for hexagon_edge in hexagon_edges:
                        if lines_intersect(square_edge, hexagon_edge):
                ...
                if collision_detected:
                    # Play the pop sound
                    pop_sound.play()
                    create_sparks_from_circle((WIDTH // 2, HEIGHT // 2), hexagon.side_length / 2, hexagon.color, sparks)
                    squares.remove(square)
                    hexagons.remove(hexagon)
                    break

    # Check if there are no squares or hexagons
    if (not squares or not hexagons) and current_time-start_time > 5:
    ...
    if not game_over:
        # Draw title
        title_text = font.render("HexaBurst Symphony", True, WHITE)
