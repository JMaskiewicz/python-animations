...
GRAVITY = 0.2  # Gravity effect magnitude
CIRCLE_GROWTH_RATE = 1  # Rate at which circles grow
...
INITIAL_CIRCLE_RADIUS = 25  # Initial radius for new circles
SPEED_INCREASE_FACTOR = 1  # Factor to increase speed after each bounce
CIRCLE_CREATION_ACCELERATION = 1.015  # Factor to decrease interval for circle creation after each bounce
...
CIRCLE_COLORS = [(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))]
GREY = (30, 30, 30)

# Ball settings
BALL_RADIUS = 15
ball_pos = [WIDTH // 2, BALL_RADIUS]  # Ball spawns at the top
...
circles = [Circle(INITIAL_CIRCLE_RADIUS + i*50, generate_random_color()) for i in range(5)]

# Trail settings
trail_positions = []
hue = 0  # Starting hue for the rainbow effect
...
# Setup video writer
video_writer = imageio.get_writer(r'C:\Users\jmask\OneDrive\Pulpit\videos\video_34\1_ball_in_circles_sound.mp4', fps=FPS)
...
while running:
    ...
    if not game_over:
        ...
        dist_x = WIDTH // 2 - ball_pos[0]
        dist_y = HEIGHT // 2 - ball_pos[1]
        distance = max(math.hypot(dist_x, dist_y), 1)  # Avoid division by zero
        ball_speed[0] += GRAVITY * dist_x / distance
        ball_speed[1] += GRAVITY * dist_y / distance
        ...
        for circle in circles:
            circle.update()
        circles = [circle for circle in circles if circle.radius <= WIDTH // 2]
        ...
        if circles:
            ...
            if current_time - last_circle_add_time >= NEW_CIRCLE_INTERVAL:
                new_circle = Circle(INITIAL_CIRCLE_RADIUS, generate_random_color())  # Start new circle with small radius
    ...
    if not game_over:
        ...
        title_text = font.render("CAN IT BREAK ALL CIRCLES???", True, WHITE)
        bounce_text = font.render(f"BOUNCES: {bounce_count}", True, WHITE)
        screen.blit(title_text, (WIDTH // 2 - title_text.get_width() // 2, 150))
        screen.blit(bounce_text, (WIDTH // 2 - bounce_text.get_width() // 2, 250))
        ...
        for i, pos in enumerate(trail_positions):
            ...
            trail_color = (int(rgb_color[0] * 255), int(rgb_color[1] * 255), int(rgb_color[2] * 255))
            pygame.draw.circle(screen, trail_color, pos, BALL_RADIUS)
...
final_audio.export(r'C:\Users\jmask\OneDrive\Pulpit\videos\video_34\1_ball_in_circles_sound.mp3', format="mp3")
...
video_clip = VideoFileClip(r'C:\Users\jmask\OneDrive\Pulpit\videos\video_34\1_ball_in_circles_sound.mp4')
audio_clip = AudioFileClip(r'C:\Users\jmask\OneDrive\Pulpit\videos\video_34\1_ball_in_circles_sound.mp3')
final_clip = video_clip.set_audio(audio_clip)
final_clip.write_videofile(r'C:\Users\jmask\OneDrive\Pulpit\videos\video_34\1_final_output.mp4', codec="libx264")
