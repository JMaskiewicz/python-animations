...
pygame.display.set_caption("Balls with Trailing Effect and Dynamic Size")
...
BALL_GROWTH_RATE = 1.025  # Growth rate of the ball
BALL_RADIUS_THRESHOLD = 100  # Threshold radius for ending the simulation
SPEED_INCREASE_FACTOR = 1.0075  # Factor to increase speed after each bounce
...
ball2_pos = [WIDTH // 2 + 100, HEIGHT // 2 + 100]
ball1_speed = [random.choice([-MAX_SPEED, MAX_SPEED]), random.choice([-MAX_SPEED, MAX_SPEED])]
ball2_speed = [random.choice([-MAX_SPEED, MAX_SPEED]), random.choice([-MAX_SPEED, MAX_SPEED])]

# Trail settings
trail1_positions = []
trail2_positions = []

# List to store hit points on the circle for each ball
hit_points_ball1 = []
hit_points_ball2 = []
...
# Function to draw lines from the circle to the center of the ball
def draw_lines(screen, hit_points, ball_pos):
...
def check_ball_circle_collision(ball_pos, ball_speed, ball_radius, circle, hit_points):
...
def check_ball_ball_collision(pos1, speed1, pos2, speed2, radius):
    distance = math.hypot(pos1[0] - pos2[0], pos1[1] - pos2[1])
    if distance < 2 * radius:
        normal = [(pos2[0] - pos1[0]) / distance, (pos2[1] - pos1[1]) / distance]
        relative_velocity = [speed2[0] - speed1[0], speed2[1] - speed1[1]]
        dot_product = relative_velocity[0] * normal[0] + relative_velocity[1] * normal[1]
        if dot_product > 0:
            return
        for i in range(2):
            impulse = 2 * dot_product * normal[i] / 2
            speed1[i] += impulse
            speed2[i] -= impulse
...
red_font = pygame.font.SysFont(None, 36)

# Setup video writer
video_writer = imageio.get_writer(os.path.join(video_dir, f'{number}_balls_in_circle_sound.mp4'), fps=FPS)
...
while running:
    ...
    if not game_over:
        ...
        ball1_speed[1] += GRAVITY
        ball1_pos[0] += ball1_speed[0]
        ball1_pos[1] += ball1_speed[1]

        ball2_speed[1] += GRAVITY
        ball2_pos[0] += ball2_speed[0]
        ball2_pos[1] += ball2_speed[1]

        # Ball collision with walls
        for ball_pos, ball_speed, ball_radius in [(ball1_pos, ball1_speed, ball1_radius), (ball2_pos, ball2_speed, ball2_radius)]:
            if (ball_pos[0] - ball_radius <= 0 or ball_pos[0] + ball_radius >= WIDTH) and not show_end_message:
                ...
                bounce_count += 1
                ball_radius = min(ball_radius * BALL_GROWTH_RATE, 200)  # Limit ball growth
        ...
        if check_ball_circle_collision(ball1_pos, ball1_speed, ball1_radius, circle, hit_points_ball1):
            bounce_count += 1
            ball1_radius = min(ball_radius * BALL_GROWTH_RATE, 200)  # Limit ball growth

        if check_ball_circle_collision(ball2_pos, ball2_speed, ball2_radius, circle, hit_points_ball2):
            bounce_count += 1
            ball2_radius = min(ball_radius * BALL_GROWTH_RATE, 200)  # Limit ball growth

        # Ball collision with each other
        check_ball_ball_collision(ball1_pos, ball1_speed, ball2_pos, ball2_speed, min(ball1_radius, ball2_radius))

        # Update trail positions
        trail1_positions.append(tuple(ball1_pos))
        if len(trail1_positions) > TRAIL_LENGTH:
            trail1_positions.pop(0)

        trail2_positions.append(tuple(ball2_pos))
        if len(trail2_positions) > TRAIL_LENGTH:
            trail2_positions.pop(0)

        # Check if ball size threshold is met
        if ball1_radius >= BALL_RADIUS_THRESHOLD or ball2_radius >= BALL_RADIUS_THRESHOLD:
    ...
    if not game_over:
        ...
        title_text = font.render("BLUE VS RED", True, WHITE)
        screen.blit(title_text, (WIDTH // 2 - title_text.get_width() // 2, 100))

        # Draw lines from hit points on the circle to the center of each ball
        draw_lines(screen, hit_points_ball1, ball1_pos)
        draw_lines(screen, hit_points_ball2, ball2_pos)

        # Draw trails
        for i, pos in enumerate(trail1_positions):
            color = TRAIL_COLORS[i % len(TRAIL_COLORS)]
            pygame.draw.circle(screen, color, pos, ball1_radius)

        for i, pos in enumerate(trail2_positions):
            color = TRAIL_COLORS[i % len(TRAIL_COLORS)]
            pygame.draw.circle(screen, color, pos, ball2_radius)

        # Draw circle
        circle.draw(screen)

        # Draw balls
        pygame.draw.circle(screen, RED, ball1_pos, ball1_radius)
        pygame.draw.circle(screen, BLUE, ball2_pos, ball2_radius)

        # Draw radii text
        radius_text1 = red_font.render(f"Red Ball: {ball1_radius:.2f}", True, RED)
        radius_text2 = red_font.render(f"Blue Ball: {ball2_radius:.2f}", True, BLUE)
        screen.blit(radius_text1, (WIDTH // 2 - radius_text1.get_width() - 100, 200))
        screen.blit(radius_text2, (WIDTH // 2 - radius_text2.get_width() + 250, 200))
...
final_audio.export(os.path.join(video_dir, f'{number}_balls_competition_sound.mp3'), format="mp3")
...
video_clip = VideoFileClip(os.path.join(video_dir, f'{number}_balls_in_circle_sound.mp4'))
audio_clip = AudioFileClip(os.path.join(video_dir, f'{number}_balls_competition_sound.mp3'))
