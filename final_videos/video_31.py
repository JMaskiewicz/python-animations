...
# Constants
FPS, MAX_SPEED, TRAIL_LENGTH, GRAVITY, ROTATION_SPEED = 60, 2, 1, 0.95, 0.02
...
class Ball:
    def __init__(self, pos, speed, color=True):
        ...
        self.speed = speed
        self.tail_length = 1
        self.trail_positions = [pos[:] for _ in range(self.tail_length)]  # Initialize with fixed length
        self.colorful = color
    ...
    def move(self):
        ...
        self.speed[0] += direction_x * GRAVITY/2
        self.speed[1] += direction_y * GRAVITY/2

        self.pos[0] += self.speed[0]
        self.pos[1] += self.speed[1]

        self.trail_positions.append(self.pos[:])  # Add a copy of the current position
        if len(self.trail_positions) > self.tail_length:
    ...
    def bounce(self):
        ...
        self.tail_length += 1
        # Adjust the trail length to accommodate the new tail length
        if len(self.trail_positions) < self.tail_length:
            self.trail_positions.extend([self.pos[:] for _ in range(self.tail_length - len(self.trail_positions))])


class Polygon:
    def __init__(self, center, radius, num_sides=20):
        self.center = center
...
# Create three concentric polygons
polygon_centers = [(WIDTH // 2, HEIGHT // 2)]
polygons = [Polygon(polygon_centers[0], radius) for radius in [360, 330, 300, 270, 240, 210, 180, 150, 120, 90, 60, 30]]

# Create the white ball in the center
white_ball = Ball([WIDTH // 2, HEIGHT // 2], [0, 0], color=False)

# Create the colorful balls
balls = [
    Ball([WIDTH // 2, 30], [random.choice([-MAX_SPEED, MAX_SPEED]), random.choice([-MAX_SPEED, MAX_SPEED])]),
    Ball([WIDTH // 2, HEIGHT - 30], [random.choice([-MAX_SPEED, MAX_SPEED]), random.choice([-MAX_SPEED, MAX_SPEED])])
]
...
while running:
            ...
            for polygon in polygons:
                ...
                for i, edge in enumerate(edges):
                    if not polygon.holes[i] and enhanced_collision_detection(ball.pos, edge):
                        ...
                        bounce_count += 1
                        ball.bounce()  # Ensure the tail length increases on collision
                        break
    ...
    screen.fill(BLACK)
    if not game_over:
        title_text = font.render("MAXIMUM GRAVITY IN THE CENTER?", True, WHITE)
