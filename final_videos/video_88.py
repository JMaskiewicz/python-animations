...
FPS = 60
POLYGON_THICKNESS = 8
ROTATION_SPEED = 0.08  # Slow rotation speed
SUBSTEPS = 10  # Increase this for finer collision checks
BALL_COUNT = 1
SPARK_COUNT = 50  # Reduced number of sparks generated on collision
NUM_POLYGONS = 4  # Number of polygons to draw
POLYGON_GAP = 75  # Gap between each polygon in radius
...
class Ball:
    def __init__(self, x, y, radius, color, growth_rate=0):
        self.x = x
        self.y = y
        self.radius = radius
        self.color = color
        ...
        self.trail = []  # List to store the trail
        self.hue = 0  # Start hue value
    ...
    def move(self, substeps):
        ...
        if len(self.trail) > 20:  # Limit the trail length
            self.trail.pop(0)

        # Increment the hue and wrap around at 360
        self.hue = (self.hue + 1) % 360

        # Convert hue to RGB color
        self.color = colorsys.hsv_to_rgb(self.hue / 360, 1, 1)
        self.color = tuple(int(c * 255) for c in self.color)

    def draw(self, screen):
        if self.alive:
            # Draw the neon trail (fading effect)
            for i, (trail_x, trail_y) in enumerate(self.trail):
                alpha = 255 - (i * 12)  # Reduce opacity for fading effect
                trail_color = (*self.color, max(alpha, 0))  # Ensure alpha doesn't go negative
                pygame.draw.circle(screen, trail_color, (int(trail_x), int(trail_y)), int(self.radius + i * 0.5))

            # Draw the black outline (slightly larger than the ball)
            pygame.draw.circle(screen, BLACK, (int(self.x), int(self.y)), int(self.radius) + 3)
...
class Spark:
    def __init__(self, x, y, vx, vy, color, initial_size, size_decrease_rate):
        self.x = x
        self.y = y
        self.vx = vx
        self.vy = vy
        ...
        self.size_decrease_rate = size_decrease_rate
        self.alive = True

    def move(self):
        self.x += self.vx
        self.y += self.vy
        self.size -= self.size_decrease_rate
        if self.size <= 0:
            self.alive = False
...
def spawn_balls(center):
    ...
    for i in range(BALL_COUNT):
        ...
        growth_rate = 0.025  # Each ball increases in size over time
        ball = Ball(x, y, INITIAL_BALL_RADIUS, color, growth_rate=growth_rate)
        balls.append(ball)
...
def create_sparks_from_polygon_edges(points, color, sparks):
    for i in range(len(points)):
        ...
        for _ in range(spark_count_per_edge):
            ...
            initial_size = random.uniform(1, 2)  # Smaller on start
            size_decrease_rate = 0.05  # Sparks decrease in size over time
            new_spark = Spark(spark_x, spark_y, vx, vy, color, initial_size, size_decrease_rate)
            sparks.append(new_spark)

# Modify the polygon rotation speed to depend on the radius
polygon_rotation_speeds = [(i/3 + 1) * ROTATION_SPEED for i in range(NUM_POLYGONS)]  # Larger polygons rotate faster

# Initialize each polygon's hue so that it's stable throughout the simulation
polygon_hues = [(i * (360 / NUM_POLYGONS)) % 360 for i in range(NUM_POLYGONS)]  # Evenly distribute hues
...
def main():
    ...
    sides = 100
    base_gap_size = sides // 15  # Adjusted base gap size
    gap_increment = sides // 50  # Each larger polygon has a larger gap
    angle_offset = 0  # Initial angle offset
    font, large_font = pygame.font.SysFont(None, 48), pygame.font.SysFont(None, 68)
    ...
    for i in range(NUM_POLYGONS):
        ...
        if radius > 0:
            ...
            gap_size = base_gap_size + (NUM_POLYGONS - 1 - i) * gap_increment
            gap_size = min(gap_size, sides)
    ...
    while running:
        ...
        title_text = font.render("BALL INCREASE IN SIZE EACH BOUNCE", True, title_color)
        screen.blit(title_text, (WIDTH // 2 - title_text.get_width() // 2, 125))

        title_text = font.render("WILL IT ESCAPE?", True, title_color)
        screen.blit(title_text, (WIDTH // 2 - title_text.get_width() // 2, 170))
        ...
        for ball in balls[:]:
            ...
            for idx in range(len(polygons) - 1, -1, -1):
                ...
                rotation_speed = polygon_rotation_speeds[idx]  # Adjust rotation speed based on polygon size

                rotated_points = rotate_polygon(points, center, direction * angle_offset * rotation_speed)
                ...
                if ball.check_collision_with_polygon(rotated_points, gap_indices):
                    ...
                    polygon_hue = polygon_hues[idx]  # Use stable hue for each polygon
        ...
        # Draw the polygons with stable colors and different rotation speeds
        for i, (points, gap_indices, radius) in enumerate(polygons):
            polygon_hue = polygon_hues[i]  # Retrieve stable hue for the polygon
            polygon_color = colorsys.hsv_to_rgb(polygon_hue / 360, 1.0, 1.0)
            polygon_color = (int(polygon_color[0] * 255), int(polygon_color[1] * 255), int(polygon_color[2] * 255))

            rotated_points = rotate_polygon(points, center, directions[i] * angle_offset * polygon_rotation_speeds[i])
        ...
        if not polygons and not show_end_message:
