...
NEW_SQUARE_INTERVAL = 0.2  # Initial time interval in seconds to add new square
...
NEW_CIRCLE_INTERVAL = 0.2  # Initial time interval in seconds to add new circle
...
class Circle:
    ...
    def get_edges(self):
        ...
        for i in range(segments):
            start_angle = angle * i
            end_angle = angle * (i + 1)
            start_pos = (center[0] + self.radius * math.cos(start_angle),
                         center[1] + self.radius * math.sin(start_angle))
            end_pos = (center[0] + self.radius * math.cos(end_angle),
...
# Setup video writer
video_writer = imageio.get_writer(rf'C:\Users\jmask\OneDrive\Pulpit\videos\video_{number}\{number}_circle_in_lines_sound.mp4', fps=FPS)
...
while running:
    ...
    if not game_over:
        ...
        # Remove circles that are too large or collide with a square
        remaining_circles = []
        for circle in circles:
        ...
        if squares:
            ...
            if current_time - last_square_add_time >= NEW_SQUARE_INTERVAL:
                ...
                rgb_color = colorsys.hsv_to_rgb(hue, 1.0, 1.0)
                square_color = (int(rgb_color[0] * 255), int(rgb_color[1] * 255), int(rgb_color[2] * 255))
                new_square = Square(700, square_color, 0.01)
...
final_audio.export(rf'C:\Users\jmask\OneDrive\Pulpit\videos\video_{number}\{number}_circle_in_lines_sound.mp3', format="mp3")
...
video_clip = VideoFileClip(rf'C:\Users\jmask\OneDrive\Pulpit\videos\video_{number}\{number}_circle_in_lines_sound.mp4')
audio_clip = AudioFileClip(rf'C:\Users\jmask\OneDrive\Pulpit\videos\video_{number}\{number}_circle_in_lines_sound.mp3')
