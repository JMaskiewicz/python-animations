...
TRIANGLE_SHRINK_RATE = 3  # Rate at which triangles shrink (adjusted to make the triangle visible longer)
...
class Triangle:
    ...
    def draw(self, screen):
        if self.side_length > MIN_TRIANGLE_SIDE:  # Adjust the condition if needed
    ...
    def update(self):
        self.side_length -= TRIANGLE_SHRINK_RATE  # Adjust the shrink rate if needed
...
while running:
    ...
    if not game_over:
        ...
        if current_time - last_triangle_add_time >= NEW_TRIANGLE_INTERVAL:
            ...
            triangles.append(Triangle(800, triangle_color, rotation_triangle))  # Start large
    ...
    if not game_over:
        # Draw title
        title_text = font.render("Spiral Illusion", True, WHITE)
