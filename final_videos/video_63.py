...
# Load MP3 file
music_path = r'C:\Users\jmask\Downloads\Gravity Falls.mp3'
...
# Constants
FPS = 60
MAX_SPEED = 7  # Maximum initial speed of hexagon
TRAIL_LENGTH = 5  # Number of trail segments (infinite in practice)
GRAVITY = 0.15  # Gravity effect
HEXAGON_SIZE = 20  # Size of the hexagon
HEXAGON_INCREMENT = 1.015  # Size increment after each bounce
CIRCLE_RADIUS_THRESHOLD = 30  # Threshold radius for ending the simulation
CIRCLE_GROWTH_RATE = 0.99  # Growth rate of the circle after each bounce
SPEED_INCREASE_FACTOR = 1.0001  # Factor to increase speed after each bounce
COOLDOWN_TIME = 0.1  # Time in seconds to wait after a collision before resizing
...
circle = Circle(340)  # Starting radius smaller to allow for growth
...
while running:
    ...
    if not game_over:
        ...
        if (hexagon_pos[0] - HEXAGON_SIZE <= 0 or hexagon_pos[0] + HEXAGON_SIZE >= WIDTH):
            ...
            # Ensure cooldown before resizing
        ...
        if circle.radius - HEXAGON_SIZE < CIRCLE_RADIUS_THRESHOLD:
